...
const StyledLinearProgress = styled(LinearProgress)(({ theme, value, error }) => ({
  height: 8,
  borderRadius: 5,
  backgroundColor: theme.palette.mode === 'dark' ? theme.palette.grey[800] : theme.palette.grey[200],
  '& .MuiLinearProgress-bar': {
    ...
    backgroundImage: error 
      ? `linear-gradient(90deg, ${theme.palette.error.light}, ${theme.palette.error.main}, ${theme.palette.error.dark})`
  ...
  '&::after': {
    ...
    animation: value === 100 || error ? 'none' : `${indeterminateAnimation} 1.8s infinite ease-in-out`,
    borderRadius: 5,
    pointerEvents: 'none',
  }
}));
...
export default function AnimatedLinearWithLabel({ value, error, ...others }) {
  const [isStarting, setIsStarting] = useState(value === 0 && !error);

  // Если значение прогресса изменилось с 0 на что-то еще или возникла ошибка,
  // переключаемся с индетерминированного на детерминированный режим
  useEffect(() => {
    if ((value > 0 || error) && isStarting) {
      setIsStarting(false);
    }
  }, [value, isStarting, error]);

  // Если передан флаг ошибки, не показываем прогресс-бар
  if (error) {
    return null;
  }
    ...
    <Box sx={{ display: 'flex', alignItems: 'center' }}>
      <Box sx={{ width: '100%', mr: 1, position: 'relative' }}>
        {isStarting ? (
          ...
          <StyledLinearProgress variant="determinate" value={value} error={error} {...others} />
        )}
      </Box>
      <Box sx={{ minWidth: 35 }}>
        <Typography 
...
AnimatedLinearWithLabel.defaultProps = {
  error: false
}; 